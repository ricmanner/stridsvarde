...
export default function KompaniDashboard() {
  ...
  useEffect(() => {
    const s = getSession();
    if (!s || s.role !== 'kompani') { router.replace('/'); return; }
    setSession(s);
  }, [router]);
  ...
  const myPlatons = KOMPANI_PLATONS[session.unit] ?? [];
  ...
  const allSoldiers = getKompaniSoldiers(session.unit, liveSoldiers);
  ...
  const catTrend = categoryTrendDataForSoldiers(allSoldiers, session.unit, period);
